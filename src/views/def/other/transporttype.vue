    ...
    <v-layout row wrap justify-space-between>
          ...
          <v-form ref="additemformref" :class="inEditMode ? 'edit-mode':''">
            ...
            <v-layout row wrap justify-space-around>
                        <v-flex xs12 sm12 md8 lg8 xl8>
                            <v-text-field v-model="FormModel.title" outlined dense :rules="rules.checkempty" label="Transport type title" dark class="pa-5"></v-text-field>
                        </v-flex>
      ...
      <v-flex xs12 sm12 md12 lg12 xl12 class="pl-10 pa-2">
        <v-card class="card-container">
          <v-card-title class="styled-card-title">Transport types</v-card-title>
          <v-card-text>
            <v-layout row wrap>
              <v-flex xs12 sm12 md12 lg12 xl12 style="padding:10px">
               <v-data-table dark :items="tariffTitleTableItems" :headers="tariffTitleTableHeaders">
                   <template v-slot:[`item.counter`]="{item}">
                       {{tariffTitleTableItems.indexOf(item)+1}}
                   </template>
...
export default {
  ...
  data:()=>({
      ...
      breadCrumb:[
          ...
          {text: 'Other',link:false},
          {text: 'Define transport type',disabled: false,href:'/other/transporttype/add'},
      ],
  ...
  methods: {
      ...
      {
          let vm = this;
          await axios.delete(URL.Other.deleteTransportType+vm.SwapId).then(async (res)=>{
      ...
      {
          let vm = this;
          await axios.get(URL.Other.transportType).then((res)=>{
                  if(MSGHNDL.resultChk(res.data))
                  {
                      console.warn(res.data.data);
                     vm.tariffTitleTableItems = res.data.data;
                  }
                  }).catch((err)=>{
                      console.error(err);
                      MSGHNDL.ERR(err.message)
      ...
      async AddItem()
      {
          ...
          if(vm.$refs.additemformref.validate())
          {
              if(!vm.inEditMode)
              {
                  await axios.post(URL.Other.transportType,vm.FormModel).then(async (res)=>{
                  if(MSGHNDL.resultChk(res.data))
                  {
                     await vm.LoadItems();
                     vm.$refs.additemformref.reset();
              ...
              {
                  await axios.put(URL.Other.transportType,vm.FormModel).then(async (res)=>{
                  if(MSGHNDL.resultChk(res.data))
                  {
                     await vm.LoadItems();
                     vm.$refs.additemformref.reset();
