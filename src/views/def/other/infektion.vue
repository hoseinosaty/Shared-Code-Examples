    ...
    <v-layout row wrap justify-space-between>
          ...
          <v-form ref="additemformref" :class="inEditMode ? 'edit-mode':''">
            ...
            <v-layout row wrap justify-space-around>
                        <v-flex xs12 sm12 md8 lg8 xl8>
                            <v-text-field v-model="FormModel.title" outlined dense :rules="rules.checkempty" label="Infektion title" dark class="pa-5"></v-text-field>
                        </v-flex>
      ...
      <v-flex xs12 sm12 md12 lg12 xl12 class="pl-10 pa-2">
        <v-card class="card-container">
          <v-card-title class="styled-card-title">Infektions</v-card-title>
          <v-card-text>
            <v-layout row wrap>
...
export default {
  ...
  data:()=>({
      ...
      breadCrumb:[
          ...
          {text: 'Define infektion',disabled: false,href:'/other/infektion/add'},
      ],
  ...
  methods: {
      ...
      {
          let vm = this;
          await axios.delete(URL.Other.deleteinfektion+vm.SwapId).then(async (res)=>{
      ...
      async AddItem()
      {
          ...
          if(vm.$refs.additemformref.validate())
          {
              if(!vm.inEditMode)
              {
                  await axios.post(URL.Other.infektion,vm.FormModel).then(async (res)=>{
                  if(MSGHNDL.resultChk(res.data))
                  {
                     await vm.LoadItems();
                     vm.$refs.additemformref.reset();
              ...
              {
                  await axios.put(URL.Other.infektion,vm.FormModel).then(async (res)=>{
                  if(MSGHNDL.resultChk(res.data))
                  {
                     await vm.LoadItems();
                     vm.$refs.additemformref.reset();
