    ...
    <v-layout row wrap justify-space-between>
        <v-flex xs12 sm12 md12 lg12 xl12 class="pa-10">
          <div class="page-divider">
              <span>Wagon status</span>
          </div>
      </v-flex>
      <v-flex xs12 sm12 md12 lg12 xl12 class="pl-10 d-inline-flex align-center">
      ...
      <v-flex xs12 sm12 md5 lg5 xl5 class="pl-10 d-flex justify-start align-center caption white--text">
          <span class="body-1 red--text pr-1">*</span> click on each color to filter wagons list
      </v-flex>
      <v-flex xs12 sm12 md12 lg12 xl12 class="pl-20">
          <v-layout row wrap>
              <v-flex v-for="(w,index) in Wagons" :key="index" xs12 sm12 md3 lg3 xl3 class="pa-2">
                  <v-sheet class="rounded-lg pa-5" style="background-color:rgba(0,0,0,.2)">
                      <v-row>
                            <v-col cols="4"><span class="caption white--text">{{w.name}}</span></v-col>
                            <v-col cols="5">
                                <v-chip style="width:100%" small :color="getStateInfo(w.workStatus).color"></v-chip>
                            </v-col>
                            <v-col cols="1"></v-col>
                            <v-col cols="2">
                                <v-btn icon small dark><v-icon small>fas fa-envelope</v-icon></v-btn>
                            </v-col>
                        </v-row>
                  </v-sheet>
              </v-flex>
...
export default {
  ...
  data:()=>({
      breadCrumb:[
          ...
          {text: 'Wagon status',disabled: false,href:'/cars/status'},
      ],
  ...
  methods: {
      FilterWagons(type)
      {
         ...
         vm.Wagons = vm.WagonRepository.filter(function(x){return x.workStatus == type});
      },
      ...
      {
          let vm = this;
          await axios.get(URL.car.byType+vm.WagonType).then((res)=>{
