          ...
          <v-form ref="additemformref" :class="inEditMode ? 'edit-mode':''">
            ...
            <v-layout row wrap justify-space-around>
                        <v-flex xs12 sm12 md8 lg8 xl8>
                            <v-text-field v-model="FormModel.title" outlined dense :rules="rules.checkempty" label="Job title" dark class="pa-5"></v-text-field>
                        </v-flex>
          ...
          <v-card-text>
            <v-layout row wrap>
              <v-flex xs12 sm12 md12 lg12 xl12 style="padding:10px">
               <v-data-table dark :items="JobtitleTableItems" :headers="JobtitleTableHeaders">
                   <template v-slot:[`item.counter`]="{item}">
                       {{JobtitleTableItems.indexOf(item)+1}}
                   </template>
...
export default {
  ...
  data:()=>({
      ...
      breadCrumb:[
          ...
          {text: 'Define Jobtitle',disabled: false,href:'/task/Jobtitle/add'},
      ],
  ...
  methods: {
      ...
      {
          let vm = this;
          await axios.delete(URL.Jobtitle.deleteItem+vm.SwapId).then(async (res)=>{
      ...
      async LoadItems()
      {
          ...
          await axios.get(URL.Jobtitle.public).then((res)=>{
                  ...
                  {
                      console.warn(res.data.data);
                     vm.JobtitleTableItems = res.data.data;
                  }
                  }).catch((err)=>{
                      console.error(err);
                      MSGHNDL.ERR(err.message)
      ...
      async AddItem()
      {
          ...
          if(vm.$refs.additemformref.validate())
          {
              if(!vm.inEditMode)
              {
                  await axios.post(URL.Jobtitle.public,vm.FormModel).then(async (res)=>{
                  if(MSGHNDL.resultChk(res.data))
                  {
                     await vm.LoadItems();
                     vm.$refs.additemformref.reset();
              ...
              {
                  await axios.put(URL.Jobtitle.public,vm.FormModel).then(async (res)=>{
                  if(MSGHNDL.resultChk(res.data))
                  {
                     await vm.LoadItems();
                     vm.$refs.additemformref.reset();
